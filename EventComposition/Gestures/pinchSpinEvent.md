...
* `Pinch` A pinch gesture recognizer reports changes to the distance between two fingers touching the screen. 
Pinch gestures are continuous, so your action method is called each time the distance between the fingers changes. <br>
* `Spin` is a speedy version of pinch <br>
`Pinch` is triggered when active touch points move.
`Spin` is triggered only when certain conditions are met, and are activated after the `touchend` event is activated. 
