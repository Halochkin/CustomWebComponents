...
describe("slottableCallBack: ", function () {
  ...
  class Slot1 extends SlottablesCallback(HTMLElement) {
    constructor() {
      super();
  ...
  class GrandpaSlot extends SlottablesCallback(HTMLElement) {
    constructor() {
      super();
      this.attachShadow({mode: "open"});
      this.shadowRoot.innerHTML = `
  ...
  class GrandGrandSlot extends SlottablesCallback(HTMLElement) {
    constructor() {
      super();
      this.attachShadow({mode: "open"});
      this.shadowRoot.innerHTML = `
  ...
  class GrandpaSlotWithSlotname extends SlottablesCallback(HTMLElement) {
    constructor() {
      super();
      this.attachShadow({mode: "open"});
      this.shadowRoot.innerHTML = `
  ...
  it("extend HTMLElement class correctly and make an element", function () {
    class SlotTestClass extends SlottablesCallback(HTMLElement) {
      constructor() {
        super();
        this.attachShadow({mode: "open"});
      }
    ...
    expect(proto.name).to.be.equal(SlottablesCallback.name);
    proto = Object.getPrototypeOf(proto);
    expect(proto.name).to.be.equal("SlottablesEvent");
  });
...
//     //only a grandchild. Such grandchild slotchange events should not trigger slotchange.
  ...
  it("removing elements also trigger callbacks, slottables, two at the same time", function (done) {
    ...
    el.innerHTML = "<div id='a'></div><div id='b' slot='something'></div>";
    requestAnimationFrame(() => {
      expect(el.testValue.length).to.be.equal(2);                    //todo slottable
      expect(el.testValue[0].slotName).to.be.equal("");
      expect(el.testValue[0].value.length).to.be.equal(1);
      expect(el.testValue[0].value[0].id).to.be.equal("a");
      expect(el.testValue[1].slotName).to.be.equal("something");
