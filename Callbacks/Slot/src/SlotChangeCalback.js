...
function doCallSlotCallback(slotchange) {
  const slot = notNipSlip(slotchange.path, this.shadowRoot);
  slot && this.slotCallback(slot);                      // it was slotCallback and did not allow to add new values to the testValue and its length never equal 2, as test expect. Now it work
}
