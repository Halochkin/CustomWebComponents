...
export const testLastPropagationTarget = {
  name: "testLastPropagationTarget",
  fun: function (res, usecase, event) {
    const usecaseFlat = usecase().flat(Infinity);
    const target = usecaseFlat[0];
    target.addEventListener(event.type, function (e) {
      const lastTarget = lastPropagationTarget(e);
      const name = eventTargetName(lastTarget);
      res.push(name);
    });
