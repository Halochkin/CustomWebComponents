...
export function processDefaultAction(event) {
  ...
  return customActions.map(({task}) => task);
  //return a list of tasks. These tasks are either run as mesoTicks or sync
}
...
function patchDefaultAction() {
  ...
  Promise.resolve().then(() => this.async = true);  //patch the async property of the native event
...
function addDefault(task, host, additive) {
  if (!this.cancelable)
    return;
  if (!host)
...
function preventDefault(nodeInContext) {
  ...
  this.preventDefults.add(contextID);
}
...
export function addDefaultAction() {
  nativePreventDefault = Object.getOwnPropertyDescriptor(Event.prototype, "preventDefault");
  nativeDefaultPrevented = Object.getOwnPropertyDescriptor(Event.prototype, "defaultPrevented");
  Object.defineProperty(Event.prototype, "preventDefault", {value: preventDefault});
  Object.defineProperty(Event.prototype, "defaultPrevented", {get: getDefaultPrevented});
  Object.defineProperty(Event.prototype, "addDefault", {value: addDefault});
  Object.defineProperty(Event.prototype, "patchDefaultAction", {value: patchDefaultAction});
}

export function removeDefaultAction() {
  delete Event.prototype.addDefault;
  delete Event.prototype.patchDefaultAction;
  Object.defineProperty(Event.prototype, "preventDefault", nativePreventDefault);
  Object.defineProperty(Event.prototype, "defaultPrevented", nativeDefaultPrevented);
}